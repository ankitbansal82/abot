// Package messenger enables interaction with arbitrary chat providers. It implements a
// standardized interface through which Telegram, Facebook M and more may be supported.
...
func Open(driverName string, r *httprouter.Router) (*Conn, error) {
	...
	driversMu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("messenger: unknown driver %q (forgotten import?)",
			driverName)
	}
